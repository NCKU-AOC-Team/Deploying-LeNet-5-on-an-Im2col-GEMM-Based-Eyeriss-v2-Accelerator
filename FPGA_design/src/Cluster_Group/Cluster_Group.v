...
// 						 		Wires  									//
// ====================================================================	//
// PE Cluster connection ???ªå‰©?ŒPE è¼¸å‡º???Œæ§??æºé ­?„æ”¤å¹??ç?ä¸­ç¹¼ç·šï?è¼¸å…¥è³‡æ?å·²ç›´??router è¼¸å‡º(è¦‹å¯¦ä¾‹å?)
// iactï¼šready ?æ? (per-row [0:2])
wire                 PECluster_iact_address_in_ready [0:2];
wire                 PECluster_iact_data_in_ready    [0:2];
// psumï¼šè¼¸??+ from_south ä¸­ç¹¼ (per-column [0:2], signed [20:0])
...
PE_Cluster PE_Cluster_inst (
	// NOTE: router<->PE ? å?äº¤å?(CSC ?½å?)ï¼šPE ??weight port å¯¦é???iact router è¼¸å‡º??
	//       iact port å¯¦é???weight router è¼¸å‡ºï¼›ä??¹ç›´??³?æ?æ­¤äº¤?‰ã€?
	.clock                   (clock),
	.reset                   (reset),
