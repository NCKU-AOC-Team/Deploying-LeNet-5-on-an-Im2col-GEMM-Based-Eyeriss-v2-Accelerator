...
本 repo **已把建好的 bitstream 一起放進版控**,所以驗收時**不必重跑合成/實作、也不必管 Vivado 版本**——
`.bit` 綁的是晶片型號（`xc7z020`),任何版本的 Vivado Hardware Manager 都能燒。權重已在
implementation 階段固化進 bitstream 的 BRAM,燒錄階段不需要任何 `.coe`。
...
- `build/` 的版控策略：為了讓驗收能直接燒,**`.bit`、`.xpr`、`.srcs/` 原始碼、最終 `*_routed.dcp`、synth `.dcp`、文字報告 `*.rpt` 會 commit**;其餘可重生的快取與中間產物（`.cache/`、`.sim/`、`.ip_user_files/`、`*_opt/_placed/_physopt.dcp`、`*.rpx` 等）由根目錄 `.gitignore` 擋掉,不要 commit。重跑時若想完全乾淨,`git clean -ndX` 先預覽再清。
...
└── NCKU_AOC_Team/
    ...
    └── build/               # Vivado 專案：.bit/.xpr/.srcs/routed.dcp/.rpt 入版控,快取與中間產物 gitignored
```
