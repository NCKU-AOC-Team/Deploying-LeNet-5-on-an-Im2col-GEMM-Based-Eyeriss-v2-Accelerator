...
def _run_qat(model_fp32, train_loader, val_loader, qconfig, epochs, lr, save_path, tag):
    ...
    # =========================================================
    # 🌟 新增：量化尺度掃描儀 (印出 Sin, Sw, Sout 與推薦硬體 Shift)
    # =========================================================
    ...
    # 尋找源頭的 Sin (來自 model.quant)
    try:
        current_sin = mq.quant.scale.item()
    except:
        current_sin = 1.0 # 如果找不到，預設給 1.0
        
    for name, layer in layers_to_analyze:
        if hasattr(layer, 'weight') and hasattr(layer, 'scale'):
            # 取得該層的 Sw (Weight Scale) 與 Sout (Output Scale)
            # 在轉換後的 quantized module 中，可以透過 q_scale() 取得
            sw = layer.weight().q_scale()
            sout = layer.scale
            
            # 計算軟體期望的真實乘數 M = (Sin * Sw) / Sout
            m_real = (current_sin * sw) / sout
            
            # 反推最完美的硬體整數 Shift N = -log2(M)
            # 加上防呆，避免 log2(0) 報錯
            if m_real > 0:
                n_ideal = -math.log2(m_real)
                n_hw = round(n_ideal)
            else:
                n_hw = "N/A"
