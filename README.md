...
- **Software Driver Development:**  
  Development of a software driver for better control and flexibility when deploying different models is a future goal. This open-source project utilizes only the PL (Programmable Logic) part of the PYNQ-Z2 board, which allows for the design of a communication interface between the PS (Processing System) and PL. This interface will handle tasks such as data allocation and batch normalization, among other related computations.
